    ...
    <div class="max-w-7xl mx-16 sm:px-6 lg:px-8">
        <nav class="text-sm" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex">
                ...
                <li class="flex items-center">
                    <a href="/admin/userenquiry" class="text-gray-700 hover:text-gray-900"> &nbsp&nbsp User Enquiry</a>
                </li>
            </ol>
        </nav>
    </div>
...
<div class="space-x-8 -my-px ms-10 flex mt-5 mb-1 pl-2">
    @if (Auth::user()->usertype == 'admin')
        <x-nav-link href="{{ route('admin.userenquiry') }}" 
        ...
        <x-nav-link href="{{ route('admin.bakeryenquiry') }}" class="hover:text-orange-500 active:text-amber-400">
        <i class="fa-solid fa-cake-candles"></i>&nbsp;&nbsp;{{ __('Bakery') }}
        </x-nav-link> 

        <!-- Search Bar -->
        <div class="searchbar hidden md:flex">
        <form action="{{ route('admin.uenquirysearch') }}" method="GET" class="flex">
    ...
    <table class="min-w-full bg-zinc-100 border-amber-500 rounded-xl">
        ...
        <tbody>
            ...
            <tr class="w-full border-gray-300 border-b">
                ...
                <a href="{{ route('admin.showuseren', ['id' => $enquiry->id, 'type' =>  'user']) }}" class="text-orange-500 hover:text-orange-700">Show</a>
                </td>
            </tr>
            @endforeach
        </tbody>
