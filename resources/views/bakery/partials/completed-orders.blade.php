    ...
    <div class="max-w-7xl mx-16 sm:px-6 lg:px-8">
        <nav class="text-sm" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex">
                ...
                <li class="flex items-center">
                    <a href="/bakery/orders/completed" class="text-gray-700 hover:text-gray-900"> &nbsp&nbsp Completed Orders</a>
                </li>
            </ol>
        </nav>
    </div>
...
<div class="space-x-8 sm:-my-px sm:ms-10 mt-5 mb-4 hidden lg:flex sm:items-center">
    @if (Auth::user()->usertype == 'bakery')
        ...
        <x-nav-link href="{{ route('bakery.orders.upcoming') }}" :active="request()->routeIs('bakery.orders.upcoming')" class="hover:text-orange-500 active:text-amber-400">
        <i class="fa-regular fa-calendar-days fa-lg" style="color: #232323;"></i>&nbsp;&nbsp;{{ __('Upcoming') }}
        </x-nav-link> 

        <x-nav-link href="{{ route('bakery.orders.completed') }}" :active="request()->routeIs('bakery.orders.completed')" class="text-orange-500 hover:text-orange-300 active:text-amber-400 border-b-4 border-orange-500 hover:border-b-4 hover:border-amber-400 active:border-b-4 active:border-red-500 pb-3 text-xl">
        ...
        <li>
            <a href="{{ route('bakery.orders.show', $order->id) }}" class="inline-flex items-center justify-between w-full p-4 py-3 text-gray-900 bg-amber-400 border-2 border-amber-400 rounded-lg cursor-pointer hover:text-gray-800 hover:bg-gray-100 hover:border-orange-500 active:border-orange-300">
                ...
                <br>
                    ...
                    <div class="w-full text-sm text-left pt-1">Completed Date: {{ $deliveryDate->format('d M Y') }}</div>
                </div>
